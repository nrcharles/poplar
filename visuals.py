...
def multi_report(domain, figname='SHS', title=None):
    ...
    soc_frequency.set_ylabel('Frequency')
    soc_frequency.set_title('Normalized SoC Histogram')
    pp = np.array(domain.log_dict_to_list('demand'))
    pp.sort()
    fit = stats.norm.pdf(pp, np.mean(pp), np.std(pp))
    print len(domain.log_dict_to_list('demand'))

    soc_frequency.hist(domain.log_dict_to_list('demand'), 40, normed=True)
    soc_frequency.plot(pp, fit)
    ...
    storage_soc.set_ylabel('hour')
    storage_soc.set_title('Storage State of Charge')
    soc = storage_soc.imshow(heatmap(domain.log_dict_to_list('demand')), aspect='auto')
    soc_bar = fig.colorbar(soc)
    soc_bar.set_label('%')
